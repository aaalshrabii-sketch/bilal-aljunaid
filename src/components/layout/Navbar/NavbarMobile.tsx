...
export function NavbarMobile({ navItems, locale, isTransparent = false }: NavbarMobileProps) {
    ...
    <div className="lg:hidden flex items-center gap-2">
      ...
      <button
        ...
        className={`p-2 transition-colors ${
          isTransparent ? 'text-white hover:text-accent' : 'text-text hover:text-accent'
        }`}
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between mb-8">
                ...
                <button
                  onClick={() => setIsOpen(false)}
                  className="p-2 text-text-secondary hover:text-accent transition-colors bg-background rounded-full border border-border"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
